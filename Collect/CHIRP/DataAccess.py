...
def RetrieveData(Date, args):
    ...
    if TimeCase == 'daily':        
        filename2 = 'chirp-v3.0.%s.%02s.%02s.tif' %(Date.strftime('%Y'), Date.strftime('%m'), Date.strftime('%d'))        
        outfilename = os.path.join(output_folder,'chirp-v3.0.%s.%02s.%02s.tif' %(Date.strftime('%Y'), Date.strftime('%m'), Date.strftime('%d')))
        DirFileEnd = os.path.join(output_folder,'P_CHIRP.v3.0_mm-day-1_daily_%s.%02s.%02s.tif' %(Date.strftime('%Y'), Date.strftime('%m'), Date.strftime('%d')))
    else:
        raise KeyError("The input time interval is not supported")
    ...
    if not os.path.exists(DirFileEnd):
        ...
        try:
            if TimeCase == 'daily':
                url = os.path.join("https://data.chc.ucsb.edu/products/CHIRP-v3.0/daily/global/tifs/%s/"  %Date.strftime('%Y'), filename2)
            else:
                raise KeyError("The input time interval is not supported")
            ...
            print("No succes to download CHIRP at %s" %Date)   
