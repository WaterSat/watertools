...
def DownloadData(input_folder, startdate, enddate, latlim, lonlim, Waitbar):
    ...
    for Date in Dates:
        ...
        for filename_svi in filenames_SVI:
            ...
            url_vnp_start = "https://ladsweb.modaps.eosdis.nasa.gov/archive/allData/5200/VNP21/%s/%s/" %(date_datetime.year, DOY)
            ...
            r = requests.get("%s.csv" %url_vnp_start)
            files_vnp = str(r.content)                
            files_vnp_hdf = [files_vnp[m.start():m.start()+40] for m in re.finditer("VNP21.A", files_vnp)]
            files_vnp_hdf_ok = [k for k in files_vnp_hdf if int(k.split(".")[2])>=int(time_str_start[0:4]) - 4 and int(k.split(".")[2])<=int(time_str_end[0:4]) + 4]
            files_vnp_hdf_ok = np.unique(files_vnp_hdf_ok)
  
            
            for idx, file_vnp_hdf_ok in enumerate(files_vnp_hdf_ok):
   
                hour = int(file_vnp_hdf_ok.split(".")[2][0:2])
                minutes = int(file_vnp_hdf_ok.split(".")[2][2:4]) 
                ...
                if not os.path.exists(filename_out):
                    ...
                    try:
                         ...
                         print("Download VNP21 from http %s" %file_vnp_hdf_ok)
                         # Download VNP03IMG
                         url_vnp = url_vnp_start + file_vnp_hdf_ok
                         Perform_Download_VIIRS(url_vnp, file_name_vnp)
                         
                         # Check if data is required
                         fh2 = Dataset(file_name_vnp, mode = "r")   
                         
                         # Open geo
                         lat = fh2.groups['VIIRS_Swath_LSTE'].groups['Geolocation Fields'].variables['latitude'][:]
                         lon = fh2.groups['VIIRS_Swath_LSTE'].groups['Geolocation Fields'].variables['longitude'][:]
                         ...
                         if len(lat_data_ok) > 0:
                             ...
                             # Open LST
                             LST = fh2.groups['VIIRS_Swath_LSTE'].groups['Data Fields'].variables['LST'][:].data
                             LST = np.float32(LST)
                             del fh2
                        ...
                        print("Error creating VIIRS file for %s" %files_vnp_hdf_ok)
...
def Perform_Download_VIIRS(url, file_name_out):
    ...
    VIIRS_BEARER, passw = watertools.Functions.Random.Get_Username_PWD.GET('NASA_BEARER')
...
def Seach_For_VIIRS(date_start, date_end, latlim, lonlim, GMT_Offset):
    ...
    putInCartPage = 'https://www.aev.class.noaa.gov/saa/products/psearchVIIRS_SDR'
    params = {}
